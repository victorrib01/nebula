  ...
  methods: {
    calculateProgress(weapons) {
      ...
      const mostProgressedWeapons = weapons
        .map((weapon) => {
          let totalCamouflages = Object.keys(weapon.progress.zombies).length
          let completedCamouflages = Object.values(weapon.progress.zombies).reduce(
            (a, b) => a + b,
            0
          )
          ...
          return {
            ...weapon,
            completed: Object.values(weapon.progress.zombies).reduce((a, b) => a + b, 0),
      ...
      // Count the required amount of camouflages to complete the Nebula camouflage
      const requiredCamouflages = mostProgressedWeapons.reduce((a, b) => {
        return a + Object.keys(b.progress.zombies).length
      }, 0)
